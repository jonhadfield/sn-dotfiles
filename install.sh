...
#   curl -fsSL .../install.sh | VERSION=0.2.0 sh

set -eu

REPO="jonhadfield/sn-dotfiles"
...
if [ "$asset_os" = "Darwin" ]; then
    ...
    asset="sn-dotfiles_Darwin_universal.tar.gz"
else
    case "$arch" in
    ...
    asset="sn-dotfiles_${asset_os}_${asset_arch}.tar.gz"
fi
...
echo "Downloading sn-dotfiles $version ($asset)"
...
tar -xzf "$tmp/$asset" -C "$tmp" sn-dotfiles || fail "archive did not contain sn-dotfiles"
chmod 755 "$tmp/sn-dotfiles"
...
if [ "$asset_os" = "Darwin" ] && command -v xattr >/dev/null 2>&1; then
    xattr -d com.apple.quarantine "$tmp/sn-dotfiles" 2>/dev/null || true
fi
...
$sudo install -m 755 "$tmp/sn-dotfiles" "$BIN_DIR/sn-dotfiles" || fail "failed to install into $BIN_DIR"

echo "Installed sn-dotfiles to $BIN_DIR/sn-dotfiles"
"$BIN_DIR/sn-dotfiles" --version
