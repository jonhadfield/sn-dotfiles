...
func TestStatus(t *testing.T) {
	...
	diffs, err = Status(session, home, []string{gitConfigPath, applePath, yellowPath, premiumPath}, true)
	assert.NoError(t, err)
...
func TestStatus1(t *testing.T) {
	...
	// add items
	var added, existing, missing []string
	added, existing, missing, err = Add(session, home, []string{gitConfigPath, awsConfig}, true)
	assert.NoError(t, err)
	...
	diffs, err = Status(session, home, []string{gitConfigPath}, true)
	assert.NoError(t, err)
